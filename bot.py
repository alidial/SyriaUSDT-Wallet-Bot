...
# 2. إعداد التوكن والآيدي المعتمد المحدث من الصور 🎯
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8859151257:AAF0SivQS_NiDcPaYiZFrt1p0Ep_T13lJTw")
ADMIN_CHAT_ID = "1843236081"  # تم التحديث للآيدي الصحيح الخاص بك من الصورة
SUPPORT_LINK = "https://t.me/Syrusdt"

# عناوين المحافظ الرسمية المحدثة بالكامل من صورك الفصيلية 🌐
MY_WALLETS = {
    "TRC20": "TAnv4K6gGk99G35uD71NymqXF2uC57xJAt",
    "BEP20": "0x7856799059F8b5123d24c0Bc3A59dfaFdD7b9815",
    "TON": "UQDbXMU9L45iztaFrwQdXMMqd6pMjsDPma4Jba_pWTRnSfEa",  # محفظة تون الافتراضية
    "SHAM_CASH": "9542037"  # رقم حساب شمس كاش الصحيح المأخوذ من صورتك
}
...
# 4. لوحة تحكم الأدمن المباشرة (/admin) للتحكم بالصرافة والنسب والعناوين
...
def admin_panel(message):
    ...
    prices_info = (
        ...
        f"اضغط على أي زر للتعديل الفوري:"
    )
...
def send_welcome(message):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("💰 حوالات وشحن المحفظة (Sham Cash)", callback_data="deposit_wallet"),
...
def handle_query(call):
    ...
    if call.data.startswith("set_") and user_id == ADMIN_CHAT_ID:
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=f"✏️ أرسل القيمة الجديدة للبند المحدد حالياً:")
        return
