...
def handle_query(call):
    ...
    if call.data.startswith("usr_net_"):
        ...
        if user_id in user_trade_steps:
            ...
            bot.edit_message_text(chat_id=chat_id, message_id=msg_id, text="🔢 يرجى إدخل الكمية المالية المطلوبة بعملة الـ **USDT**\n(أرقام فقط، الحد الأدنى 5 USDT، مثال: 50):", parse_mode="Markdown", reply_markup=markup)
        return
...
# --- قسم التشغيل المعدل لحل مشكلة 409 Conflict ---
if __name__ == '__main__':
    init_db()
    
    # 1. تشغيل خادم الويب الخاص بـ Render في خلفية منفصلة لضمان استمرار السيرفر
    threading.Thread(target=start_isolated_web_server, daemon=True).start()
    
    # 2. تنظيف وحذف أي اتصالات أو ويبهوك معلق قديم لإزالة التعارض فوراً
    try:
        logger.info("جاري حذف الـ Webhook القديم وتنظيف الروابط لتفادي التعارض الحركي...")
        bot.remove_webhook()
    except Exception as e:
        logger.error(f"خطأ أثناء حذف الـ Webhook: {e}")

    # 3. بدء استقبال الرسائل الآمن والمستقر مع إسقاط التراكمات القديمة
    logger.info("بدء تشغيل استقبال الرسائل (Polling) بنجاح وبشكل مستقر...")
